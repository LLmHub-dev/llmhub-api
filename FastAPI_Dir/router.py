...
from utils.database import load_mongo_uri, initialize_mongo_client, get_mode_config, get_sys_prompt_config, write_sys_prompt_config
...
def load_api_key(api_key_path='api_key.txt'):
    ...
    try:
        ...
        logging.error(f"API key file not found: {e}")
        raise
...
def generate_prompt(models, intent):
    ...
    try:
        updated_prompt_data = create_dynamic_prompt(prompt_data, intent, models)
        logging.info("Dynamic prompt generated successfully.")
        return updated_prompt_data["system_prompt"]
    except Exception as e:
